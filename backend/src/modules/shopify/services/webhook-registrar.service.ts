...
const TOPICS: readonly { topic: string; path: string }[] = [
  ...
  /*
   * The wake-up call for a job parked on a bulk operation. Shopify recommends
   * this over polling, and without the subscription every such job waits for
   * the coordinator's sweep instead of resuming the moment Shopify is done.
   */
  {
    topic: 'BULK_OPERATIONS_FINISH',
    path: '/webhooks/bulk-operations-finish',
  },
];
