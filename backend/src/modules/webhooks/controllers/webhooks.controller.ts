...
export class WebhooksController {
  ...
  constructor(
    ...
    private readonly bulkOperations: BulkOperationCoordinator,
  ...
  /**
   * `bulk_operations/finish` — a bulk query or mutation has stopped.
   *
   * This is the wake-up call for every job parked in WAITING_BULK. Shopify
   * recommends the webhook over polling, and for us it is the difference
   * between a job resuming seconds after Shopify finishes and one waiting for
   * the next poll tick.
   *
   * The payload carries only `admin_graphql_api_id` and `status` — no results.
   * That is enough: the operation row is found by that id, the status recorded,
   * and the job woken to fetch its own JSONL.
   *
   * Fires for failures and cancellations too, not only completions. A job whose
   * operation failed still has to run again to record that and give up
   * honestly; nothing else will ever notify it.
   */
  @Post('bulk-operations-finish')
  @HttpCode(200)
  async bulkOperationsFinish(
    @Req() req: RawBodyRequest<Request>,
    @Headers('x-shopify-hmac-sha256') hmac: string | undefined,
    @Headers('x-shopify-shop-domain') shopDomain: string | undefined,
    @Headers('x-shopify-webhook-id') webhookId: string | undefined,
    ...
    return this.handle(
      req,
      { hmac, shopDomain, webhookId, topic: 'bulk_operations/finish' },
      async () => {
        const payload = safeJson(req.rawBody);
        const shopifyId = payload['admin_graphql_api_id'];
        if (typeof shopifyId !== 'string') {
          this.logger.warn(
            'bulk_operations/finish carried no admin_graphql_api_id',
          );
          return;
        }
        await this.bulkOperations.handleFinished(
          shopifyId,
