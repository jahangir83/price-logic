...
/**
 * Which field on the row each rung of the ladder looks up.
 *
 * `SUPPLIER_SKU` is deliberately looked up against the store's **SKU** field
 * rather than anything of its own: there is no per-supplier mapping table yet,
 * and plenty of merchants simply use their supplier's code as their SKU. That
 * is the case this rung rescues.
 */
const IDENTIFIERS: Record<MatchStrategy, (row: CsvRow) => string | null> = {
  [MatchStrategy.SKU]: (row) => row.sku,
  [MatchStrategy.SUPPLIER_SKU]: (row) => row.supplierSku,
  [MatchStrategy.BARCODE]: (row) => row.barcode,
};
...
export class ImportsService {
  ...
  async match(shop: Shop, importId: string): Promise<CsvImport> {
    ...
    // The same floor activation will apply, so a pre-filled price on the
    // approval screen is one the campaign can actually write.
    const { minimumPrice } = resolveStoreSettings(shop.defaultSettings);

    /*
     * The ladder. Each rung sees only the rows every rung before it failed, so
     * a sheet that matches entirely on SKU costs exactly what it always did —
     * the extra lookups happen only for rows that would otherwise have come
     * back unmatched and cost the merchant a manual fix.
     *
     * Ambiguity is fatal at every rung, not just the first: two variants
     * sharing a barcode is the same "we cannot tell which" as two sharing a
     * SKU, and guessing at the bottom of the ladder is no safer than guessing
     * at the top.
     */
    const pending = new Map(rows.map((row) => [row.id, row]));
    const ambiguous = new Map<string, number>();

    for (const strategy of MATCH_LADDER) {
      if (pending.size === 0) break;

      const identifierOf = IDENTIFIERS[strategy];
      const candidates = [...pending.values()].filter(
        (row) => identifierOf(row) !== null,
      );
      ...
      const matches =
        strategy === MatchStrategy.BARCODE
          ? await this.shopify.findVariantsByBarcode(shop, values)
          : await this.shopify.findVariantsBySku(shop, values);
      const byValue = new Map(
        matches.map((match) => [match.sku, match.variants]),
      );

      for (const row of candidates) {
        const found = byValue.get(identifierOf(row) ?? '') ?? [];
        if (found.length === 0) continue;

        if (found.length > 1) {
          // Remembered, not applied yet: a later rung may still identify this
          // row unambiguously, and a clean barcode match is a better answer
          // than "your SKUs are duplicated".
          ambiguous.set(row.id, found.length);
          continue;
        }

        this.applyMatch(row, found[0], strategy, shop, campaign, minimumPrice);
    ...
    for (const row of pending.values()) {
      ...
      row.matchedBy = null;
      /*
       * Cleared, not just left. Matching runs again whenever a sheet is
       * re-matched, and a row that resolved last time but not this one would
       * otherwise keep pointing at a variant it no longer matches — a record
       * saying UNMATCHED while still naming a product. Activation filters on
       * status so nothing would be mispriced, but a row that contradicts
       * itself is one nobody can trust later.
       */
      ...
      const duplicates = ambiguous.get(row.id);
      row.errorMessage = duplicates
        ? `${duplicates} products share that identifier, so it is not clear which to update.`
        : 'No product in this store matches this row.';
    }
  ...
  /** Copies one resolved variant onto the row. */
  private applyMatch(
    row: CsvRow,
    variant: VariantPriceRecord,
    ...
    row.matchedBy = strategy;
    row.shopifyVariantId = variant.variantId;
    row.shopifyProductId = variant.productId;
    row.productTitle = variant.productTitle;
    row.variantTitle = variant.variantTitle;
    row.stockQuantity = variant.inventoryQuantity;
    row.currentPrice = variant.price;
    row.currency = shop.currency;
    row.approvedPrice = this.computeApprovedPrice(row, campaign, minimumPrice);
  }
  ...
  async listRows(
    ...
    if (query.sort === CsvRowSort.CHANGE) {
      /*
       * Biggest mover first, by proportion rather than by amount — 50p is
       * nothing on a sofa and a third of the price of a mug.
       *
       * Rows with nothing to compare go last rather than being dropped: an
       * unmatched row is still one the merchant has to deal with, and hiding
       * it behind a sort order is how it gets missed. The division is exact
       * decimal, because these columns are numeric(19,4) — no float enters it.
       */
      builder
        .addSelect(
          `CASE
             WHEN row.current_price IS NULL
               OR row.current_price = 0
               OR COALESCE(row.approved_price, row.sheet_price) IS NULL
             THEN NULL
             ELSE ABS(
               (COALESCE(row.approved_price, row.sheet_price) - row.current_price)
               / row.current_price
             )
           END`,
          'change_ratio',
        )
        .orderBy('change_ratio', 'DESC', 'NULLS LAST')
        // A stable tie-break, so paging through a sheet where every price moved
        // by the same percentage does not shuffle rows between pages.
        .addOrderBy('row.row_number', 'ASC');
    } else {
      builder.orderBy('row.row_number', 'ASC');
    }
   ...
   *
   * Without this an import is reachable only by its id, which the merchant
   * never sees written down — so a sheet uploaded yesterday is simply gone,
   * along with any campaign still waiting on its approval.
   */
  async listImports(
    shopId: string,
    query: { supplierId?: string; page?: number; pageSize?: number } = {},
  ): Promise<PaginatedResponse<CsvImport>> {
    const page = query.page ?? 1;
    const pageSize = query.pageSize ?? DEFAULT_PAGE_SIZE;

    const builder = this.imports
      .createQueryBuilder('import')
      .where('import.shop_id = :shopId', { shopId });

    if (query.supplierId) {
      builder.andWhere('import.supplier_id = :supplierId', {
